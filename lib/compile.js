...
function renderSass(input, sassOpt, options) {
    ...
    const p = gulp
        ...
        .pipe(footer('\n/*! compile at:' + new Date().toISOString() + ' */'))
    ...
    const withMin = typeof options.withMin == 'function' ? options.withMin(input) : options.withMin;
...
function miniCss(input, options = {}) {
    const sourcemaps = options.map;
    const outpath = options.outpath || './';
    var p = gulp
        .src(input)
        .pipe(postcss([autoprefixer, cssnano]))
...
function miniJs(input, options = {}) {
    const sourcemaps = options.map;
    const outpath = options.outpath || './';
    const watch = options.watch;
    if (watch) {
        const w = gulp.watch([input], {
            ignoreInitial: true,
            events: ['add', 'change']
        });
        w.on('all', function (type, filePath) {
            log(gray(type), filePath);
            return miniJs(filePath, { map: sourcemaps, outpath });
        });
        return w;
    }
    const p = gulp
        .src(input, { sourcemaps })
        .pipe(
            uglify({
                output: { comments: /\/\*\!/i },
                ie8: true
        ...
        .pipe(gulp.dest(outpath, { sourcemaps: '.' }))
        .on('finish', function () {
            log(gray(input), success('minify success'));
        });
...
function watchJs(globs = ['**/*.js'], options) {
    if (!Array.isArray(globs)) {
        globs = [globs];
    }
...
module.exports = {
    ...
    renderSass: function (input, min, options) {
        const output = {
            withMin: min,
            outpath: options.outpath
        };
        if (options.watch) {
            return watchSass(input, {}, output);
        }
        return renderSass(input, {}, output);
    },
    mini: function (inputs, options) {
        const jsArr = [];
        const cssArr = [];
        inputs.forEach((it) => {
            if (/\.js$/.test(it)) {
                jsArr.push(path.resolve(process.cwd(), it));
            } else if (/\.css$/.test(it)) {
                cssArr.push(path.resolve(process.cwd(), it));
            } else {
                log(error(it, '不支持的文件类型'));
            }
        });
        jsArr.forEach((it) => miniJs(it, options));
        cssArr.forEach((it) => miniCss(it, options));
    }
};
